...
# number of keeping wordnet
MAX_WORDNET_CANDIDATES = 12

# min show time to calculate
MIN_DOC_FREQ = 2
...
ALPHA_TARGET = 0.42 #Target level fit
ALPHA_SEM = 0.36    #Semantic similarity
ALPHA_CONTEXT = 0.16 #Local fluency
ALPHA_FREQ = 0.06    #Target-level frequency
...
def _try_init_nltk():
    """
    initialize NLTK
    import nltk / wordnet / stopwords / WordNetLemmatizer
    """
...
def _try_init_spacy():
    """
    initialize spaCy and pyinflect
    """
...
def analyze_sentence(sentence: str) -> List[Dict[str, object]]:
    ...
    # use fallback if fail to use spaCy
    parts = []
...
class BigramLM:
    ...
    def score(self, tokens: List[str]) -> float:
        """
        Compute the average log probability of a token sequence.
        The higher the score, the more natural the sequence is under the language model.
        """
        if not tokens:
            return 0.0
        prev = "<s>"
...
class ResourceBundle:
    """
    All resources learned during training are stored here.

    These include:
    - occurrence statistics of each word across different CEFR levels
    - total frequency of each word
    - word frequency within each CEFR level
    - the dominant part of speech of each word
    - a separate bigram language model for each CEFR level
    - the final estimated CEFR difficulty score for each word
    """
    ...
    def _build(self):
        """
        Build statistics from data.csv.
        """
...
def generate_candidates(
    ...
    """
    generate candidates by WordNet
    """
    bundle = get_resources()
